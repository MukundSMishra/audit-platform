...
/**
 * Fetch all submitted/completed audit sessions
...
export async function fetchAllAuditSessions() {
  try {
    // Fetch from audit_sessions table where status is submitted or completed
    const { data: sessions, error } = await supabase
      .from('audit_sessions')
      .select('id, factory_name, location, submitted_at, audit_type, risk_score, status')
    ...
    if (!sessions || sessions.length === 0) {
      console.warn('[Report Service] No submitted audit sessions found');
      return [];
    }

    console.log('[Report Service] Fetched audit sessions:', {
